...
async function runSQL(filePath: string, connection: mysql.Connection): Promise<void> {
  try {
...
export async function initializeDatabase(): Promise<void> {
  ...
  try {
    ...
    if (exists) {
      ...
      console.log(chalk.yellow.bold(`Database ${DATABASE_NAME} doesn't exist...`));
    }
    ...
    console.log(chalk.red.bold("Failed to connection to SQL database:"), error);
  }
}
